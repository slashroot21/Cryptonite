...
class SellForm extends Component {
  ...
  render() {
    const tokenList = [
      ...
      { value:'Neon', label: 'Neon', image: "https://t4.ftcdn.net/jpg/04/61/15/19/360_F_461151906_tOlOeEqCwWtvPR1UY0BCSJQTCr3O4SLK.jpg", balance: '400',rate: '125'},
      {value: "Xenon", label: "Xenon", image: "https://images.unsplash.com/photo-1654020233909-bcbd71d2ebc7?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwcm9maWxlLXBhZ2V8Nnx8fGVufDB8fHx8&w=1000&q=80", balance: '700',rate: '160'}
    ];
    ...
    return (
      <form onSubmit={(event) => {
        ...
        if(this.state.token==="Krypt") {
          ...
          this.setState({balance: ('' + (parseInt(this.state.balance) - this.input.value))})
        }
      }} >
        <Stack spacing={1}>
                ...
                <Input
                  ...
                  onChange={(event) => {
                    ...
                    this.setState((prevState)=> ({
                      output: tokenAmount / parseInt(prevState.rate)
                    }))
                  }}
                  ref={(input) => { this.input = input }}
                />
                <Text fontSize="small" color="white" as="samp">Balance:{this.state.balance}</Text>
              </NumberInput>
          ...
          <Stack spacing={3}>
            <Stack
              ...
              <Text as="samp" fontSize="small" color={'red.400'}>{this.state.rate} {this.state.token} = 1 ETH </Text>
            </Stack>
